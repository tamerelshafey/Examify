...
const ExamFormModal: React.FC<ExamFormModalProps> = ({ isOpen, onClose, onSave }) => {
  ...
  const handleAiAssist = async (qIndex: number) => {
      ...
      try {
          ...
          newQuestions[qIndex] = {
              ...
              type: QuestionType.MultipleChoice, // AI assist generates MCQs
          };
          setQuestions(newQuestions);
      } catch (error) {
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const finalQuestions = questions.map((q, index) => {
      ...
      // For ordering questions, the correct answer is the sequence of options
          ...
          <div className="space-y-6">
            {questions.map((q, qIndex) => (
              <div key={qIndex} className="bg-slate-50 dark:bg-slate-700 p-6 rounded-lg border border-slate-200 dark:border-slate-600 relative">
                <button type="button" onClick={() => removeQuestion(qIndex)} className="absolute top-3 right-3 text-red-500 hover:text-red-700">
                    ...
                    <div className="space-y-2">
                        {q.options?.map((opt, oIndex) => (
                            <div key={oIndex} className="flex items-center gap-2">
                                ...
                                <button type="button" onClick={() => removeListItem(qIndex, oIndex)} className="text-red-500 hover:text-red-600 disabled:opacity-50" disabled={q.options && q.options.length <= 2}>
                ...
                {q.type === QuestionType.MultipleSelect && (
                    <div className="space-y-2">
                        {q.options?.map((opt, oIndex) => (
                            <div key={oIndex} className="flex items-center gap-2">
                                <input type="checkbox" name={`correct-answer-${qIndex}`} value={opt} 
                                    ...
                                    onChange={e => {
                                        ...
                                        handleQuestionChange(qIndex, 'correctAnswer', newAnswers);
                                    }} />
                                <input type="text" placeholder={`${t.options} ${oIndex + 1}`} value={opt} onChange={e => handleOptionChange(qIndex, oIndex, e.target.value)} className="p-2 bg-white dark:bg-slate-600 rounded-md w-full" required/>
                                <button type="button" onClick={() => removeListItem(qIndex, oIndex)} className="text-red-500 hover:text-red-600 disabled:opacity-50" disabled={q.options && q.options.length <= 2}>
                                    <XCircleIcon className="w-5 h-5" />
                 ...
                 {q.type === QuestionType.TrueFalse && (
                    <div className="flex gap-4">
                        <label className="flex items-center"><input type="radio" name={`correct-answer-${qIndex}`} value="True" checked={q.correctAnswer === 'True'} onChange={e => handleQuestionChange(qIndex, 'correctAnswer', 'True')} className="me-2" /> True</label>
                        <label className="flex items-center"><input type="radio" name={`correct-answer-${qIndex}`} value="False" checked={q.correctAnswer === 'False'} onChange={e => handleQuestionChange(qIndex, 'correctAnswer', 'False')} className="me-2" /> False</label>
                    </div>
                )}
                 {q.type === QuestionType.TrueFalseWithJustification && (
                    <div>
                        ...
                        <textarea placeholder={t.modelJustification} value={q.correctJustification || ''} onChange={e => handleQuestionChange(qIndex, 'correctJustification', e.target.value)} className="p-2 bg-white dark:bg-slate-600 rounded-md w-full h-24" />
                    </div>
                )}
                {(q.type === QuestionType.Essay || q.type === QuestionType.ShortAnswer) && (
                    <textarea placeholder={t.modelAnswerHelp} value={q.correctAnswer as string} onChange={e => handleQuestionChange(qIndex, 'correctAnswer', e.target.value)} className={`p-2 bg-white dark:bg-slate-600 rounded-md w-full ${q.type === QuestionType.ShortAnswer ? 'h-16' : 'h-24'}`} required />
                )}
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400">{t.orderingHelp}</p>
                         {q.options?.map((opt, oIndex) => (
                            <div key={oIndex} className="flex items-center gap-2">
                                ...
                                <input type="text" placeholder={`${t.addItem} ${oIndex + 1}`} value={opt} onChange={e => handleOptionChange(qIndex, oIndex, e.target.value)} className="p-2 bg-white dark:bg-slate-600 rounded-md w-full" required/>
                                <button type="button" onClick={() => removeListItem(qIndex, oIndex)} className="text-red-500 hover:text-red-600 disabled:opacity-50" disabled={q.options && q.options.length <= 2}>
                                    <XCircleIcon className="w-5 h-5" />
                                </button>
                            </div>
                        ))}
                        <button type="button" onClick={() => addListItem(qIndex)} className="text-sm text-primary-500 hover:text-primary-600 font-semibold mt-2">{t.addItem}</button>
                    </div>
                )}
                        ...
                        <div>
                            ...
                            {q.prompts?.map((prompt, pIndex) => (
                                <div key={pIndex} className="flex items-center gap-2 mb-2">
                                    <input type="text" placeholder={`${t.prompts} ${pIndex+1}`} value={prompt} onChange={e => handleOptionChange(qIndex, pIndex, e.target.value, 'prompts')} className="p-2 bg-white dark:bg-slate-600 rounded-md w-full" required/>
                                    <button type="button" onClick={() => removeListItem(qIndex, pIndex, 'prompts')} className="text-red-500 hover:text-red-600 disabled:opacity-50" disabled={q.prompts && q.prompts.length <= 1}><XCircleIcon className="w-5 h-5"/></button>
                                </div>
                            ))}
                             <button type="button" onClick={() => addListItem(qIndex, 'prompts')} className="text-sm text-primary-500 hover:text-primary-600 font-semibold mt-2">{t.addPrompt}</button>
                        </div>
                        <div>
                            <h4 className="font-semibold mb-2">{t.options}</h4>
                            {q.options?.map((opt, oIndex) => (
                                <div key={oIndex} className="flex items-center gap-2 mb-2">
                                    <input type="text" placeholder={`${t.options} ${oIndex+1}`} value={opt} onChange={e => handleOptionChange(qIndex, oIndex, e.target.value, 'options')} className="p-2 bg-white dark:bg-slate-600 rounded-md w-full" required/>
                                    <button type="button" onClick={() => removeListItem(qIndex, oIndex, 'options')} className="text-red-500 hover:text-red-600 disabled:opacity-50" disabled={q.options && q.options.length <= 1}><XCircleIcon className="w-5 h-5"/></button>
                                </div>
                            ))}
                             <button type="button" onClick={() => addListItem(qIndex, 'options')} className="text-sm text-primary-500 hover:text-primary-600 font-semibold mt-2">{t.addOption}</button>
                        </div>
                        <div className="col-span-2">
                            <h4 className="font-semibold mb-2">{t.correctMatches}</h4>
                            {q.prompts?.map((prompt, pIndex) => (
                                <div key={pIndex} className="flex items-center gap-4 mb-2">
                                    <span className="w-1/3">{prompt || `${t.prompts} ${pIndex+1}`}</span>
